	...
	jQuery.fn.pagebox_colorpicker = function() {
		...
		return this.each(function() {
			...
			jQuery(colorpicker).wpColorPicker({
				palettes: [ '#ffffff', '#1A9CE3', '#002664', '#ce0000', '#A2A4A3', '#00A599', '#D7A900', '#BA6F2E' ]
			});
		});
	...
	jQuery.fn.pagebox_repeater = function() {
		...
		return this.each(function() {
			...
			jQuery(repeater).on('click', '.pagebox[data-action="repeater_add"]', function(e){
				...
				var new_field = jQuery( '<table>' + group + '</table>' );
				jQuery(this).parents().eq(3).after( new_field );
				...
				jQuery(repeater).parents('form').pagebox_form();
				jQuery(repeater).find('.make-chosen').chosen({
					allow_single_deselect: true,
					width: '100%',
				});

				jQuery( this ).closest( '.element-repeater' ).trigger( 'pagebox/repeater/added', [ new_field ] );
